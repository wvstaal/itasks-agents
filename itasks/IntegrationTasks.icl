...
where
	...
	eval event repOpts state=:(TCBasic taskId lastEvent encv stable) iworld=:{world}
		| stable
			= (ValueResult (Value (fromJust (fromJSON encv)) True) {TaskInfo|lastEvent=lastEvent} (TaskRep (UIControlSequence {UIControlSequence|attributes='Map'.newMap,controls=[],direction=Vertical}) defaultValue) state, iworld)
		| otherwise
			= case fromJSON encv of
				Just (Right outfile)
					...
					| not exists
						...
						# rep				= TaskRep (UIControlSequence (layout.Layout.interact prompt editor)) defaultValue
					...
					= case fromJSON (fromString (fromOk res)) of
						...
						Just async	
							| async.AsyncResult.success
								...
								= (ValueResult (Value result True) {TaskInfo|lastEvent=lastEvent} (TaskRep (UIControlSequence {UIControlSequence|attributes = 'Map'.newMap,controls = [],direction = Vertical}) defaultValue) (TCBasic taskId lastEvent (toJSON result) True), {IWorld|iworld & world = world})
							| otherwise
